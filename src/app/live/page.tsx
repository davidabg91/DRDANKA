          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7 items-stretch">
            {LIVE_COURSES.map((c) =>
              c.card.cover ? (
                ...
                <InfoCourseCard key={c.slug} course={c} price={resolvePrice(c.slug, overrides, c.priceEur)} />
              )
            )}
          </div>
        )}
...
/** Footer shared by both card variants: price + "Виж" CTA. */
function CoursePriceFooter({ course, price }: { course: LiveCourse; price: number }) {
  return (
      ...
      <div>
        ...
        <div className="flex items-end gap-2">
          <span className="font-serif text-2xl sm:text-3xl font-bold text-brand-gold leading-tight">
            {price.toFixed(2)}<span className="text-sm text-brand-dark/50 font-sans ml-0.5">€</span>
          </span>
...
/** Catalog card for courses with a cover photo. */
function PhotoCourseCard({ course: c, price }: { course: LiveCourse; price: number }) {
  return (
    <Link
      href={`/live/${c.slug}`}
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
          <span className="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-wider backdrop-blur-sm px-2.5 py-1 rounded-full shadow-sm bg-white/95 text-brand-green">
      ...
      <div className="p-5 sm:p-6 flex flex-col flex-1 gap-3">
        ...
        <CoursePriceFooter course={c} price={price} />
      </div>
    </Link>
  );
}
...
/**
 * Catalog card for courses without a cover photo. No image area at all —
 * the whole panel is information: icon + badge header, title, tagline,
 * concrete highlights (so the card reads as content-rich, not "missing a photo").
 */
function InfoCourseCard({ course: c, price }: { course: LiveCourse; price: number }) {
  const Icon = c.card.icon || BookOpen;
  return (
    <Link
      href={`/live/${c.slug}`}
      className="group relative bg-white rounded-3xl border border-brand-green/10 hover:border-brand-gold/50 shadow-md hover:shadow-2xl hover:shadow-brand-gold/20 hover:-translate-y-1 transition-all duration-300 flex flex-col cursor-pointer overflow-hidden"
    >
      {/* Slim brand-gold edition strip instead of a photo */}
      <div className="h-1.5 bg-gradient-to-r from-brand-gold via-brand-gold-light to-brand-gold shrink-0" />

      <div className="p-5 sm:p-6 flex flex-col flex-1 gap-4">
        <div className="flex items-start justify-between gap-3">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-brand-green to-[#0D2B1C] text-brand-gold flex items-center justify-center shrink-0 shadow-md group-hover:scale-105 transition-transform duration-300">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-brand-dark/60 leading-relaxed line-clamp-2">{c.tagline}</p>
        </div>

        {c.highlights && c.highlights.length > 0 && (
          <ul className="space-y-1.5 border-t border-brand-green/5 pt-3">
            {c.highlights.map((h) => (
              <li key={h} className="flex items-start gap-2 text-[11px] text-brand-dark/70 leading-snug">
                <Check className="h-3 w-3 text-brand-gold shrink-0 mt-0.5" strokeWidth={3} /> {h}
              </li>
            ))}
          </ul>
        )}
