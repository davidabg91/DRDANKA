        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative overflow-hidden rounded-[2rem] border border-brand-gold/25 bg-gradient-to-br from-[#0A1F18] via-[#0D2B1C] to-[#081410] text-white shadow-2xl">
            ...
            <div className="relative z-10 p-5 sm:p-7 lg:p-9">
              {/* ─── Header row ─── */}
              <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3 pb-5 border-b border-white/10">
                <div className="max-w-2xl">
                  <span className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.2em] bg-brand-gold text-brand-dark px-3 py-1.5 rounded-full shadow-lg shadow-brand-gold/20">
                    <Star className="h-3 w-3" fill="currentColor" /> Премиум пакет
                  </span>
                  <h2 className="font-serif text-2xl sm:text-3xl lg:text-[2.5rem] font-bold mt-3 leading-[1.05] tracking-tight">
                    Проверка преди проверката
                  </h2>
                  <p className="text-white/65 text-sm leading-relaxed mt-2">
                    Независим професионален одит на предприятието Ви <span className="text-brand-gold">преди инспекцията на БАБХ</span> — открийте слабите места първи, вместо да чакате инспектора.
                  </p>
                </div>
                {/* Risk pills */}
                <div className="flex flex-wrap gap-1.5 lg:justify-end lg:max-w-xs">
                  {["Предписания", "Актове", "Спиране на дейност", "Блокирана продукция", "Изтегляне от пазара"].map((r, i) => (
                    <span key={i} className="inline-flex items-center gap-1 text-[10px] font-semibold text-red-200/90 bg-red-950/40 border border-red-500/25 rounded-full px-2.5 py-1">
                      <XCircle className="h-3 w-3 text-red-400 shrink-0" /> {r}
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div className="grid lg:grid-cols-12 gap-6 lg:gap-8 pt-6">
                {/* LEFT — what's included + deliverables */}
                <div className="lg:col-span-8 space-y-6">
                  <div>
                    <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-brand-gold mb-3">
                      <Search className="h-4 w-4" /> Какво включва проверката
                    </h3>
                    <ul className="grid sm:grid-cols-2 gap-x-6 gap-y-1.5">
                      {[
                        "Внедрената НАССР система",
                        "Разработените ДПХП",
                        ...
                        <li key={i} className="flex items-start gap-2 text-[13px] text-white/80 leading-snug py-0.5">
                          <Check className="h-3.5 w-3.5 text-brand-gold shrink-0 mt-[3px]" strokeWidth={3} />
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="grid sm:grid-cols-2 gap-5 pt-1">
                    <div className="bg-white/[0.05] border border-white/10 rounded-xl p-4">
                      <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-white/90 mb-2.5">
                        <FileCheck className="h-4 w-4 text-brand-gold" /> Получавате
                      </h3>
                      <ul className="space-y-1.5">
                        {[
                          "Подробен писмен доклад с несъответствията",
                          "Анализ на риска за предприятието",
                          "План с коригиращи действия",
                          "Практически препоръки за подобрение",
                          "Консултация за резултатите",
                        ].map((d, i) => (
                          <li key={i} className="flex items-start gap-2 text-[13px] text-white/75 leading-snug">
                            <CheckCircle className="h-3.5 w-3.5 text-brand-gold shrink-0 mt-[3px]" />
                            <span>{d}</span>
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="bg-gradient-to-br from-brand-gold/20 to-brand-gold/[0.04] border border-brand-gold/35 rounded-xl p-4">
                      <div className="inline-flex items-center gap-1.5 text-brand-gold text-[10px] font-black uppercase tracking-widest mb-1.5">
                        <Gift className="h-3.5 w-3.5" /> Бонус
                      </div>
                      <p className="font-serif text-lg font-bold text-white leading-tight">30 дни подкрепа</p>
                      <p className="text-white/60 text-xs leading-relaxed mt-1.5">След проверката задавате въпроси по изпълнението на препоръките:</p>
                      <ul className="mt-2 space-y-1 text-xs text-white/80">
                        <li className="flex items-center gap-1.5"><Check className="h-3 w-3 text-brand-gold shrink-0" /> Без лимит на въпросите</li>
                        <li className="flex items-center gap-1.5"><Check className="h-3 w-3 text-brand-gold shrink-0" /> Без допълнително заплащане</li>
                        <li className="flex items-center gap-1.5"><Phone className="h-3 w-3 text-brand-gold shrink-0" /> Телефон и имейл</li>
                      </ul>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-4">
                  <div className="bg-white/[0.07] border border-brand-gold/30 rounded-2xl p-5 backdrop-blur-sm lg:sticky lg:top-24">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-brand-gold/80">Инвестиция</span>
                    <div className="flex items-baseline gap-1.5 mt-0.5">
                      <span className="font-serif text-5xl font-black text-white tabular-nums leading-none">600</span>
                      <span className="font-serif text-2xl font-bold text-brand-gold">€</span>
                      <span className="text-white/50 text-xs ml-1">еднократно</span>
                    </div>
                    <Link
                      href={`/contact?service=${encodeURIComponent("Проверка преди проверката")}`}
                      className="w-full mt-4 inline-flex items-center justify-center gap-2 px-6 py-3.5 text-xs font-black uppercase tracking-widest rounded-xl bg-brand-gold hover:bg-brand-gold-light text-brand-dark shadow-lg shadow-brand-gold/25 hover:-translate-y-0.5 transition-all duration-300 cursor-pointer"
                    >
                      Заявете проверка <ChevronRight className="h-4 w-4" />
                    </Link>
                    <ul className="mt-4 space-y-1.5 text-[11px] text-white/55 leading-relaxed border-t border-white/10 pt-3">
                      <li className="flex items-start gap-1.5"><MapPin className="h-3 w-3 text-brand-gold/70 shrink-0 mt-0.5" /> Цена за обекти до 50 км; при по-далече — транспортни разходи.</li>
                      <li className="flex items-start gap-1.5"><FileText className="h-3 w-3 text-brand-gold/70 shrink-0 mt-0.5" /> За големи обекти над един работен ден — индивидуална оферта.</li>
                      <li className="flex items-start gap-1.5"><Clock className="h-3 w-3 text-brand-gold/70 shrink-0 mt-0.5" /> Посещението се резервира след предварително заплащане.</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              {/* ─── Signature ─── */}
              <div className="mt-6 border-t border-white/10 pt-4 flex items-start gap-3">
                <ShieldCheck className="h-5 w-5 text-brand-gold shrink-0 mt-0.5" />
                <p className="text-white/60 text-[13px] leading-relaxed font-serif italic">
                  „Бивш директор на Областна дирекция по безопасност на храните с над 25 години опит в официалния контрол. Най-добрата проверка е тази, за която вече сте подготвени.“
                  <span className="not-italic text-brand-gold font-bold text-xs ml-1.5 font-sans">— д-р Данка Николова</span>
                </p>
              </div>
            </div>
          </div>
